...
async function main(): Promise<void> {
  const server = createOnlineServer({ port: 0, host: '127.0.0.1' })
  ...
  const black = await openSocket(url)
  ...
  if (state.board[to]?.player !== 'you' || state.board[from] !== null) {
    throw new Error('Authoritative move was not applied correctly')
  }
  ...
  console.log('Online smoke: create/join/turn-validation/move-sync PASS')
}
